...
function updateData() {
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      // (userID != uid ? ( document.getElementById('verify').innerHTML = 'verified') :  document.getElementById('verify').innerHTML = 'not verified') 
...
firebase.auth().onAuthStateChanged((user) => {
  if (user) {
    document.getElementById("stID").value=user.uid;
